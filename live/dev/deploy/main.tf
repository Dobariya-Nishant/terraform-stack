locals {
  ...
  ecs_cluster  = data.terraform_remote_state.compute.outputs.ecs_cluster
}
...
module "backend_service" {
  ...
  project_name           = var.project_name
  environment            = var.environment
  capacity_provider_name = local.ecs_cluster.asg_cp["backend"].name
  name                   = "backend"
  ecs_cluster_id         = local.ecs_cluster.id
  ecs_cluster_name       = local.ecs_cluster.name
  desired_count          = 1
...
module "frontend_service" {
  ...
  project_name           = var.project_name
  environment            = var.environment
  capacity_provider_name = local.ecs_cluster.asg_cp["frontend"].name
  name                   = "frontend"
  ecs_cluster_id         = local.ecs_cluster.id
  ecs_cluster_name       = local.ecs_cluster.name
  desired_count          = 1
